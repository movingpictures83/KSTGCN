...
def load_szassist_data(dataset, inputfile):
    sz_adj = pd.read_csv(inputfile+"/sz_adj.csv",header=None)
    adj = np.mat(sz_adj)
    data = pd.read_csv(inputfile+"/sz_speed.csv")
...
def preprocess_data(data1, time_len, train_rate, seq_len, pre_len,methods,attribute, inputfile):
    train_size = int(time_len * train_rate)        
    train_data = data1[0:train_size]
    test_data = data1[train_size:time_len]     
    trainX, trainY, testX, testY = [], [], [], []
    sz_poi = pd.read_csv(inputfile+"/sz_poi.csv",header = None)
    sz_poi = np.transpose(sz_poi)
    sz_poi_max = np.max(np.max(sz_poi))
    sz_poi_nor = sz_poi/sz_poi_max
    sz_weather = pd.read_csv(inputfile+"/sz_weather_all.csv",header = None)
...
#        weather_embedding = pd.read_csv('data/sz_weather_embedding.csv',header = None)
#        weather_embedding = abs(np.transpose(weather_embedding))
#        kg_embedding = pd.read_csv('data/sz_kg_embedding.csv',header = None)
